...
class YT_CLEAN_DATSET(Dataset):
    ...
    def initialize(self, opt, mode=''):
        ...
        split_list = open(os.path.join(opt.data_root, f"YT-Clean.{mode}.1.lst")).read().splitlines()
        self.data_fp = os.path.join(opt.data_root, "frames")
        data_root = os.listdir(self.data_fp)
        self.vid = [vid for vid in data_root if vid in split_list]
        self.audio_path = os.path.join(opt.data_root, "cipic_subj3")
        self.frame_path = os.path.join(opt.data_root, "frames")

        self.audios = []
        for vid_ in self.vid:
            audio_path = os.path.join(self.data_fp, vid_, "cipic_subj3")
            audio_files = sorted(os.listdir(audio_path))[:-1]
            self.audios.extend(
                [os.path.join(audio_path, audio_file) for audio_file in audio_files]
            )
    ...
    def _load_image(self, frame_path, frame_index):
        frames = Image.open(os.path.join(frame_path, str(frame_index).zfill(6) + '.jpg')).convert('RGB')
    ...
    def _load_image_tta(self, frame_path, frame_index, augment=False, crop_w=448,  crop_h=224):
        image_path = os.path.join(frame_path, str(frame_index).zfill(6) + '.jpg')
    ...
    def prepare_visual(self, frame_path, clip_info=None, audio_len=None, windows_list=None, audio_num_samples=None, vid_time=None):
        if self.mode == 'train':
            ...
            frame_index = frame_index + vid_time * 10
            frame_index = np.clip(frame_index, a_min=1, a_max=vid_time * 10 + 99)
            ...
            num_frames = len(glob(os.path.join(frame_path, '*.jpg')))
    ...
    def load_batch(self, index):
        audio_fp = self.audios[index]
        file_id = os.path.split(audio_fp)[-1].split(".wav")[0]

        vid_time = int(os.path.split(audio_fp)[-1].split(".wav")[0])
        audio, clip_info, audio_clips, windows_list = self.prepare_audio(audio_fp)
        ...
        video_fp = "/".join(audio_fp.split("/")[:-2])
        video_fp = os.path.join(video_fp, "video")
        frames, masks = self.prepare_visual(video_fp, **kwgs)
