...
class PatchEmbed(nn.Module):
    ...
    def __init__(
        self, img_size=(46, 30), dim_in=1280, embed_dim=768, norm_layer=None
    ):
        super().__init__()
...
class Block(nn.Module):
    def __init__(
        ...
        self.forward_method = (
            self.forward_ca if mode == "CA" else self.forward_sa
        )

    def SDPAttention(self, q, k, v, attn_mask=None, size=None):
        output, attn_ = self.drop_path(
            self.attn(q, k, v, attn_mask=attn_mask, size=size)
        )
        q = q + output
...
class CROSS_ATTENTION(nn.Module):
    ...
    def forward(self, f_v, f_a, attn_mask=None, size=None):
        ...
        for layer in self.blocks:
            ...
            f_a, attn_a = layer(f_in, attn_mask=attn_mask, size=size)
        # f_v = self.norm(f_v)
        f_a = self.norm(f_a)
        return f_a, attn_a
